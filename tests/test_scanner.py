...
class TestScanResult:
    ...
    def test_is_success_true(self):
        ...
        result = ScanResult(url="https://example.com", status_code=200)
        assert result.is_success is True

    def test_is_success_false_with_error(self):
        """Test failed scan with error."""
        result = ScanResult(url="https://example.com", error="Connection failed")
...
class TestSecurityHeaderScanner:
    ...
    def test_scanner_initialization(self):
        ...
        scanner = SecurityHeaderScanner(
            timeout=5, follow_redirects=False, verify_ssl=False, user_agent="TestAgent"
        )
